...
class TestServer(TestCase):
    ...
    def test_request_presence(self):
        response = self.server.generate_response(Request(RequestAction.PRESENCE))
        response.time = 1.1
        self.assertEqual(response, self.response_ok)

    def test_request_quit(self):
        response = self.server.generate_response(Request(RequestAction.QUIT))
        response.time = 1.1
        self.assertEqual(response, self.response_ok)

    def test_request_message(self):
        response = self.server.generate_response(Request(RequestAction.MESSAGE))
        response.time = 1.1
        self.assertEqual(response, self.response_basic)

    def test_request_incorrect(self):
        response = self.server.generate_response(Request('test'))
        response.time = 1.1
        self.assertEqual(response, self.response_incorrect)

    def test_request_empty(self):
        response = self.server.generate_response(Request(None))
        response.time = 1.1
        self.assertEqual(response, self.response_incorrect)

    def test_request_presence_conflict(self):
        self.server.generate_response(Request(RequestAction.PRESENCE, 'test'))
        response = self.server.generate_response(Request(RequestAction.PRESENCE, 'test'))
        response.time = 1.1
